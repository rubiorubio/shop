...
class CreateFeatureView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not created:
            return JsonResponse({
                "error": f"The value '{value}' already excist."
            })
        messages.add_message(
            request, messages.SUCCESS,
            f'Value "{value}" for Characteristic '
            f'"{feature.feature_name}" in category {category.name} successefly created'
        )
...
class ShowProductFeaturesForUpdate(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        <hr>
            <div class="row">
                <div class="col-md-4">
                    <h4 class="text-center">Characteristic</h4>
                </div>
                <div class="col-md-4">
                    <h4 class="text-center">Current value</h4>
                </div>
                <div class="col-md-4">
                    <h4 class="text-center">New value</h4>
                </div>
            </div>
        <div class='row'>{}</div>
        <div class="row">
        ...
        <div class="col-md-4">
            <p class='text-center'><button class="btn btn-success" id="save-updated-features">Save</button></p> 
        </div>
        <div class="col-md-4">
        </div>
        </div>
...
class UpdateProductFeaturesAjaxView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.add_message(
            request, messages.SUCCESS,
            f'The value for the CHaracteristic  {product.title} are succesfuly  updated'
        )
        return JsonResponse({"result": "ok"})
